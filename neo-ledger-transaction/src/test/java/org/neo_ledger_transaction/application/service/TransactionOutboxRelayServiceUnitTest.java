...
class TransactionOutboxRelayServiceUnitTest {
    ...
    private final TransactionOutboxPublisherPort publisherPort = mock(TransactionOutboxPublisherPort.class);
    ...
    void execute_should_publish_and_mark_as_processed() {
        TransactionOutboxMessage message = new TransactionOutboxMessage(
                UUID.randomUUID(),
                "SEPA_PAIN_001",
                new byte[]{1, 2, 3},
                0
    ...
    void execute_should_schedule_retry_on_publish_failure() {
        ...
        when(queryPort.claimDueMessages(any(), eq(20))).thenReturn(List.of(message));
        doThrow(new RuntimeException("broker down")).when(publisherPort)
                .publish(any(), any(), any());
    ...
    void execute_should_wrap_state_failure() {
        ...
        when(queryPort.claimDueMessages(any(), eq(20))).thenReturn(List.of(message));
        doThrow(new RuntimeException("broken state")).when(statePort)
                .markAsProcessed(any(), any());
