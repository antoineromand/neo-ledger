 ...
 *
 * <p>It claims due messages, delegates publication, and updates the outbox state through output
 * ports. The class intentionally contains no JPA or Kafka details.
 */
public class TransactionOutboxRelayService implements TransactionOutboxRelayUseCasePort {
  ...
  public void execute() {
    ...
    try {
      messages =
          this.transactionOutboxQueryPort.claimDueMessages(
              LocalDateTime.now(this.clock), BATCH_SIZE);
    ...
    for (TransactionOutboxMessage message : messages) {
      try {
        this.transactionOutboxPublisherPort.publish(
            message.routingKey(), message.id().toString(), message.payload());
      } catch (RuntimeException exception) {
      ...
      try {
        this.transactionOutboxStatePort.markAsProcessed(
            message.id(), LocalDateTime.now(this.clock));
  ...
  private void handleFailure(TransactionOutboxMessage message, RuntimeException exception) {
    ...
    try {
      this.transactionOutboxStatePort.scheduleRetry(
          message.id(), nextRetryCount, nextAttemptAt, error);
    } catch (RuntimeException stateException) {
      throw new TransactionOutboxStateException(stateException);
    }
  }
  ...
  private LocalDateTime computeNextAttemptAt(int retryCount) {
    long delayMinutes =
        Math.min(BASE_BACKOFF.toMinutes() * (1L << Math.max(0, retryCount - 1)), 30L);
