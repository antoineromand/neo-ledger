 ...
 *
 * <p>This implementation uses the StAX (Streaming API for XML) API to parse XML files efficiently.
 * Unlike DOM, the document is not loaded entirely into memory, ensuring high performance for large
 * files.
 *
 * <p>The parser extracts:
 *
 * <ul>
 *   <li>Global file information ({@code GrpHdr})
 *   <li>Payment information blocks ({@code PmtInf})
 *   <li>Individual transaction details ({@code DrctDbtTxInf})
 * </ul>
 *
 * <p>The result is wrapped in a {@link ParsedPaymentFile} containing:
 *
 * <ul>
 *   <li>A {@link ParsedFileHeader}
 *   <li>A list of {@link ParsedSepaTransaction} objects
 * </ul>
 */
@Component
public class SepaPain008Parser
   ...
   * @return A parsed representation of the file, including header and transactions.
   ...
   *
   * <p>This block contains the global metadata for the SEPA file:
   *
   * <ul>
   *   <li>Message identifier
   ...
   *
   * <p>Information in this block is shared across all transactions it contains (Creditor IBAN,
   * collection date, creditor identifier).
   *
      ...
      if (event == XMLStreamConstants.START_ELEMENT) {
        switch (r.getLocalName()) {
          ...
          case "DrctDbtTxInf" ->
              transactions.add(
                  parseTransaction(
                      r, creditorIban, groupIsInstant, requestedDate, creditorSchemeId));
        }
    ...
    while (r.hasNext()) {
      ...
      if (event == XMLStreamConstants.START_ELEMENT) {
        switch (r.getLocalName()) {
          ...
          && "DrctDbtTxInf".equals(r.getLocalName())) {
        break;
      }
    }
    ...
    while (r.hasNext()) {
      ...
      if (event == XMLStreamConstants.START_ELEMENT && "Othr".equals(r.getLocalName())) {
        creditorSchemeId = parseOtherIdentification(r);
          ...
          && "CdtrSchmeId".equals(r.getLocalName())) {
        break;
      }
    }
